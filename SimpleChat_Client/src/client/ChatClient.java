...
public class ChatClient extends AbstractClient {
    ...
    public ChatClient(String host, int port, ChatIF clientUI) {
        ...
        log.debug("ChatClient's Cunstructor initializes.");
        try {
            log.info(host+" try to open connection.");
            openConnection();   //콘솔 프로그램 시작 시 서버에의 연결 시도
        } catch (IOException e) {
            log.error("openConncetion have IOException. next phase is automatic #lofoff", e);
    ...
    public void handleMessageFromServer(Object msg) {   //서버에서의 메시지를 받으면 무조건으로 콘솔에 표시
        log.info("received message from the server.");
        clientUI.display(msg.toString());
    }
    
    public void handleMessageFromClientUI(String message) { //자신이 입력하는 메시지에 대해서
        log.info("received input from myself.");
        if (message.startsWith("#login") ) {
            log.info("It is #login input. trying to open connection.");
            try {
                openConnection();   //연결 시도
            } catch (IOException e) {   //로그인 시 연결을 실패했을 경우
                log.error("openConncetion have IOException.", e);
        ...
        if (message.startsWith("#quit")){    //종료 명령어인 경우 그에 대한 메소드 실행
            log.info("It is #quit input. trying to open connection."); 
            quit();
        }

        if (message.startsWith("#logoff"))  {   //로그오프에 대한 명령어인 경우
            log.info("It is logoff input. trying to close connection.");
            try {
                ...
                log.error("closeConncetion have IOException.", e);
                quit();
            }
            return;
        }
        ...
        if (message.startsWith("#sethost")) {   //호스트 변경 명령어인 경우
            if (isConnected())  {  //연결이 있는 경우 호스트가 변경되지 않도록(요구사항)
                ...
                log.warn("stil have connection so that you cannot change host");
                return;
            }  
            //연결이 없는 경우
            try {
                log.info("host will be changed to "+message.substring(9));
        ...
        if (message.startsWith("#setport")) {   //포트 번호 변경 명령어인 경우
            if (isConnected()) { //똑같이 연결이 있을 경우 변경되지 않도록(요구사항)
                ...
                log.warn("stil have connection so that you cannot change port");
                return;
            }
        ...
        if (message.startsWith("#help") || message.startsWith("#?")) {
            log.info("client side help() execute");
        ...
        if ((!(message.startsWith("#")))    //일반 메시지거나 서버의 도움이 필요한 명령어들의 경우
            ...
            log.info("It is the command the server should help with.");
            try {
                sendToServer(message);  //위 종류들의 메시지들은 서버로 전송하여 처리함.
                log.debug("sending to server is success.");
                ...
                log.error("sending to server is fail.");

                try {
                    log.debug("trying to close connection because sendging to server fail.");
                    closeConnection();
                    log.debug("closing connection because sendging to server fail is success.");
                } catch (IOException ex) {
                    log.error("not only sent, but also close is failed.");
                    clientUI.display("Cannot logoff normally.  Terminating client.");
                    quit();
                }
            }
        ...
        //잘못된 # 명령어의 메시지들.
        log.warn("client entered invalid command.");
        clientUI.display("Invalid command.");
    }

    public void quit() {    //종료 과정(연결 끊기->시스템 종료)
        log.info("It is a closing Connection by client.");
        try {
            closeConnection();
        } catch (IOException e) { 
            log.error("closing Connection by client is fail.");
        }
        log.info("anyway, System exit.");
        System.exit(0);
    }

    protected void connectionClosed(boolean isAbnormal) {   //연결을 끊는 경우 그것이 정상인지 비정상 해제인지를 클라이언트에게 메시지로 표시하는 메소드
        log.info("client\'s connection closed().");
        if (isAbnormal)  {
    ...
    protected void connectionEstablished() {    //openConnection()이 정상적으로 이루어졌을 때 클라이언트에게 표시할 수 있는 메시지로 사용되는 ocsf 메소드 구현
        log.info("client\'s connection established().");
    ...
    protected void connectionException(Exception exception) {  //서버가 연결을 끊었을 때의 메시지로 사용되는 ocsf 메소드 구현
        log.info("client\'s connection exception().");
