import java.io.*;
import common.*;
import server.*;

import org.apache.logging.log4j.Logger;
import org.apache.logging.log4j.LogManager;

public class ServerConsole implements ChatIF {
    //대부분 private static. 
    //Log4j2 클래스를 사용한 것이지만 어떤 초기화 방법을 선택하든 Logger 인스턴스가 있으면 로그 이벤트 생성할 수 있음.
    private static Logger log = LogManager.getLogger(ServerConsole.class);

    final public static int DEFAULT_PORT = 5555;

    EchoServer server;  //서버 콘솔은 에코 서버를 가지고 있음

    public ServerConsole(int port) throws IOException { //서버도 콘솔 입력이 가능해지도록 함에 따라 에코서버와 콘솔 사이의 문맥이 교환되고 콘솔 생성자에서 서버 객체가 같이 생성되도록 됨.
        server = new EchoServer(port, this);
        log.debug("ServerConser's Cunstructor initializes.");
    }

    public void display(String message) {
        System.out.println(message);
    }

    public void accept() {  
        try {
            BufferedReader fromConsole = new BufferedReader(new InputStreamReader(System.in));
            String message;

            try {
                while (true) {  //서버도 콘솔의 입력을 받기 위해 무한 반복
                    message = fromConsole.readLine();
                    server.handleMessageFromServerUI(message);
                }
            } catch (NullPointerException e) { }
        } catch (Exception ex) {    //널포인터 예외를 제외한 예외 상황의 에러 스택 출력 및 메시지 표시
            log.error("ERROR!", ex);
        }
    }

    public static void main(String[] args) {
        log.info("ServerConsole Program start.");
        int port = 0;
        try {   //포트 번호를 파라미터로 받거나 기본 포트 사용
            port = Integer.parseInt(args[0]);
            log.info("args[0] exist: "+port);
        } catch (Throwable t) {
            port = DEFAULT_PORT;
            log.info("args[0] does not exist: "+port, t);
        }

        try {
            ServerConsole sv = new ServerConsole(port); //서버 콘솔 객체 생성 및 입력 대기
            sv.accept();
        } catch (IOException e) {
            log.error("Could not start listening for clients.", e);
        }
    }
}
